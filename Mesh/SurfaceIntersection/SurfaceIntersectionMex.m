function [intMatrix, intSurface] = SurfaceIntersectionMex(surface1, surface2, lookuptable)
...
if nnz(tMsk)>0
  ...
  V = zeros(length(face1), size(V1,2), 3);
  U = zeros(length(face2), size(U1,2), 3);
  ...
  [intMatrix(tMsk), intSurface2] = TriangleIntersection2D(V, U, ...
    N1(face1,:), getIntersection, lookuptable, debug);
...
if(getIntersection && n>0)
  ...
  tempfaces = [1:n; n+1:2*n; n+1:2*n]';
  intSurface = struct('vertices', [p1; p2], ...
                      'faces', tempfaces, ...
                      'edges', tempfaces(:,1:2));
...
function [overlap, intSurface] = TriangleIntersection2D(V, U, N, ...
  getIntersection, lookuptable, debug)
...
%     clear V2d U2d
  ...
  if getIntersection && overlap(row)
    ...
    if isempty(order) % when one tri is totally contained in the other tri then order is set
      order = IntersectionPolygon(edgeMat>0, points, dIdx, lookuptable, debug);
...
%% ========================================================================
function polygon = IntersectionPolygon(edgeMat, points, dIdx, lookuptable, debug)
...
if (lookuptable.LUTcount(key) == 0)
  return
end
polygon = lookuptable.orderLUT(key, 1:lookuptable.LUTcount(key));
...
doPlot = (~lookuptable.verified(key));
...
for i = 1:nx+6
  text(points(i,1)+d(1), points(i,2)+d(2), points(i,3), thePainfulInt2Str(i))
end
