function [K_, u_ext_] = LQRTrajFollow(F_,G_,H,R,Q2,y_star,x0,N)
%LQRTrajFollow Run discrete-time LQR to solve trajectory follow problem
%   F,G: x(t+1) = F(t)x(t) + G(t)u(t) t=0~N-1
%   H: y(t) = H'(t)x(t)               t=1~N
...
for j = 1:N-1
    t = N - j; %1~N-1
    SG_ = S(:,:,t+1) * G_(:,:,t+1);
    GSGRG_(:,:,t+1) = ( G_(:,:,t+1)' * SG_ + R(:,:,t+1) )^-1*G_(:,:,t+1)';
    GSGRG_SF(:,:,t+1)= GSGRG_(:,:,t+1)* S(:,:,t+1)* F_(:,:,t+1);
    temp2= S(:,:,t+1)* F_(:,:,t+1) - SG_ * GSGRG_SF(:,:,t+1); 
    S(:,:,t) = F_(:,:,t+1)'* temp2 + Q(:,:,t);
end
GSGRG_(:,:,1) = ( G_(:,:,1)' * S(:,:,1) * G_(:,:,1) + R(:,:,1) )^-1*G_(:,:,1)';
GSGRG_SF(:,:,1)= GSGRG_(:,:,1)* S(:,:,1)* F_(:,:,1);
